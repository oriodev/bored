import { SetTutorialVisible } from "@/types";
...
const Tutorial = ({ visible, setTutorialVisible }: TutorialProps) => {
  return (
    <div 
      className={`${visible ? 'absolute' : 'hidden'} w-full h-full bg-black/60 hover:cursor-pointer`} 
      onClick={() => setTutorialVisible(false)}
    >
      <div className={`flex flex-col gap-5 items-center w-2/3 h-1/2 bg-pink-200 p-10 rounded-lg text-black absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 overflow-hidden overflow-y-scroll`}>
        {/* HEADING */}
        ...
        <p>click anywhere to close the tutorial</p>
      </div>
    </div>
  )
}
